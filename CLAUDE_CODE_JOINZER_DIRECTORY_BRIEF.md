...
where status='draft' and provenance->>'origin'='locations' and name_source_url is null;
```
Work/Cowork research + publish these next (same routing as Phoenix). The overloaded `locations.access_type` legacy enum is **frozen** (comment-documented; new venue writes use the unified `facility_listings` vocab — access_type + `fee_type` + `indoor`); the ADR-13 #2 map applied to the parity rows was `public→public`, `fee_based→public+fee_type=fee` (walk-in commercial = public access, paid), `indoor_public→public+indoor`, `resort→public+fee (provisional; "Resort" note)`, `private/semi_private→private`, `hoa→hoa`. **Deprecated (marked, NOT dropped)**: `locations.category/source_url/notes/phone` (superseded by facility_listings; read-path switch is Phase 3B). Promotion **flow** (app write-path cutover) is Phase 3B.
