  ...
  if (!token || loadError) {
    return (
      <main className="max-w-sm mx-auto px-4 py-12 text-center space-y-3">
        ...
        <Link href="/compete" className="text-sm text-brand-active underline">Browse leagues</Link>
      </main>
    )
  }
  ...
  if (outcome === 'accepted') {
    return (
      <main className="max-w-sm mx-auto px-4 py-12 text-center space-y-3">
        ...
        <Link href={`/compete/leagues/${invitation.league_id}`} className="text-sm text-brand-active underline">
          View league →
        </Link>
      </main>
    )
