                  ...
                  <>
                    ...
                    <div className="flex gap-3 mt-1 flex-wrap">
                      {(s.status === 'completed' || s.status === 'in_progress') && (
                        <Link href={`/leagues/${leagueId}/sessions/${s.id}/results`} className="text-xs text-brand-active underline underline-offset-2">
                          Results →
                        </Link>
                      )}
                      {s.status === 'in_progress' && (
                        <Link href={`/leagues/${leagueId}/sessions/${s.id}/live`} className="text-xs text-brand-active underline underline-offset-2">
                          Live →
                        </Link>
                      )}
                      {s.status === 'scheduled' && (
                        <Link href={`/leagues/${leagueId}/sessions/${s.id}/results`} className="text-xs text-brand-muted underline underline-offset-2">
                          Enter results →
                        </Link>
                      )}
                      <button
