...
export function teamLabel(regId: string | null, regs: OrgRegistration[]): string {
  ...
  // pre-gated at the data layer), prefixed once for both singles and doubles.
  const seed = r.display_seed != null ? `#${r.display_seed} ` : ''
  ...
  if (r.partner_registration_id) {
    ...
    if (a && b) return `${seed}${[a, b].sort((m, n) => m.localeCompare(n)).join('/')}`
    return `${seed}${a || b || r.team_name || regId.slice(0, 8)}`
  }
  return `${seed}${r.team_name || firstName(r.player_name) || regId.slice(0, 8)}`
}
