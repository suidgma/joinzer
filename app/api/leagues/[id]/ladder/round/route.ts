...
// Generate the next king-of-the-court round for the active ladder session. Round 1
// seeds present entrants onto courts of two by ladder rank; later rounds apply
// up-down movement from the just-scored round (which must be fully scored first).
// Court games + byes are stored as league_fixtures. Organizer/co-admin.
export async function POST(_req: NextRequest, props: Params) {
  ...
  let nextMatchNum = fx.length ? Math.max(...fx.map((f) => f.match_number ?? 0)) + 1 : 1
  ...
  if (currentRound === 0) {
    ...
    const roundFx = fx.filter((f) => f.match_stage === 'ladder_round' && f.round_number === currentRound)
    const unscored = roundFx.filter((f) => f.status !== 'completed').length
    if (unscored > 0) {
      return NextResponse.json({ error: `Score all ${roundFx.length} courts in round ${currentRound} first.`, incomplete: unscored }, { status: 409 })
    }
    ...
    const byeRow = fx.find((f) => f.match_stage === 'ladder_bye' && f.round_number === currentRound)
  ...
  return NextResponse.json({ ok: true, round: targetRound })
}
