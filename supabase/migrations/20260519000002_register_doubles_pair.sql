-- Ticket 1.2: register_doubles_pair RPC
-- Creates two linked registrations atomically for a doubles team (organiser-add path).
-- Mirrors the solo auto-match cross-link pattern from register/route.ts lines 148-157.
--
-- Capacity note: the solo auto-match path (route lines 129-223) does not check capacity;
-- that check lives at route lines 80-108. Because no upstream guard exists for this RPC
-- path, we use a conservative slot count: COUNT(*) WHERE status='registered' >= max_entries*2
-- (each registered team produces exactly 2 rows). Flagged for future hardening.
...
BEGIN
  ...
  -- Safe placeholder — see note at top of file.
