  ...
  return (
    <label className="flex items-center justify-between gap-3 text-xs cursor-pointer">
      ...
      <span className="flex items-center gap-2 shrink-0">
        <span className={`w-6 text-right text-[11px] font-semibold ${checked ? 'text-brand-active' : 'text-gray-400'}`}>
          {checked ? 'On' : 'Off'}
        </span>
        <button
          type="button"
          ...
          className={`relative h-5 w-9 shrink-0 rounded-full border transition-colors ${checked ? 'bg-brand border-brand' : 'bg-gray-300 border-gray-400'}`}
        >
      ...
      {open && (
        <div className="border-t border-brand-border px-4 py-4 space-y-4">
          ...
          <div className="border-t border-brand-border pt-3 space-y-2.5">
            ...
            <Toggle label="Schedule by division priority" checked={draft.schedule_by_priority} onChange={v => set('schedule_by_priority', v)} />
