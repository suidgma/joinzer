...
- **Organizer onboarding — Phase 1 shipped (July 8, 2026, PRs #302–#305)** — the funnel + guided first-event path now exists. `profiles.signup_intent` (`play|organize|both`, migration `20260708000005`) captured via a required "What brings you to Joinzer?" step in profile setup; `/organizers` gained a co-primary **"Start free — create your first event"** CTA (hybrid with the Calendly demo) that carries `intent=organize` through login + the OAuth callback into setup; after setup an organizer routes to a new **`/get-started`** (League-vs-Tournament cards + how-it-works) instead of the player-first home; and the empty `/home` leads a declared-organizer-who-hasn't-created-yet with a "Create your first event" card. **Phase 2 (not built):** make the tournament `SetupChecklist` items clickable, add a **league** setup checklist (none today), and a post-create "you're live — next steps" card. **Phase 3:** organizer-mode signaling, a getting-started resource, leagues "+ Create" button parity. Reality note: this is self-serve scaffolding; the #1 open decision (a real organizer conversation) is still unbooked.
...
*Last verified against repo: July 8, 2026*
