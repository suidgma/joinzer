...
- **Box leagues + unified attendance (July 3, 2026, PRs #225–#227)** — **Box leagues** (`leagues.format_kind='box'`) run tiered boxes per **cycle** (`league_periods` / `league_boxes` / `league_box_members`), round-robin `league_fixtures`, promotion/relegation, and per-cycle standings — seeded/scored on the Roster + Standings pages. The round-robin **attendance grid + substitute flow** (Here / Coming / Late / Can't Come / Sub / Not Here) is now **shared across formats**: extracted into `components/features/leagues/AttendanceGrid.tsx` (driven by a normalized `AttendeeRow[]` + the pure `buildAttendeeRows` sub-overlay resolver in `lib/leagues/attendance.ts`) and backed by a format-agnostic **`league_attendance`** table (occasion = session XOR cycle `period_id`; attendee = registration / user / guest; RLS deny-all + service role). Box gets its own attendance surface at `/leagues/[id]/attendance`, reached via the format-aware **Run Session** nav action (`lib/leagues/runSession.ts` `getRunSessionAction`) — members grouped by box, add/assign subs; a sub's fixture results credit the covered **registration** so standings/promotion stay correct with no scoring change. Round-robin still stores attendance on `league_session_players` (unchanged — the storage was **intentionally not** migrated onto `league_attendance`; rationale in `docs/phases/unified-attendance.md`). v1 attendance/subs are **entrant-level** (a team in doubles, a player in singles). Box design: `docs/phases/league-formats.md`.
...
*Last verified against repo: July 3, 2026*
