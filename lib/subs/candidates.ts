...
// Returns up to `limit` opted-in, eligible, not-yet-notified candidates for the request, best first.
// Ranking here is light (prior participant in this league, then present) — the full ranking lives in
// matching.ts for the requester-facing feed; the fan-out only needs a sensible bounded top slice.
export async function loadEligibleCandidatesForRequest(requestId: string, opts: { limit?: number } = {}): Promise<SubCandidate[]> {
  const limit = opts.limit ?? 15
  const db = admin()

  const { data: req } = await db
    .from('league_sub_requests')
    .select(`id, status, league_id, league_session_id, league_period_id, requesting_player_id, gender_required, expires_at,
  ...
  if (r.expires_at && new Date(r.expires_at).getTime() <= Date.now()) return []
  ...
  if (isSession ? formatKind !== 'session_rr' : (formatKind !== 'box' && formatKind !== 'ladder')) return []

  // Occasion must not have started / been generated.
  if (isSession) {
    if (['completed', 'cancelled'].includes(session?.status)) return []
    if (await hasRows(db, 'league_rounds', 'session_id', r.league_session_id)) return []
  } else {
    if (period?.status !== 'active') return []
    if (await hasRows(db, 'league_fixtures', 'period_id', r.league_period_id)) return []
  }
  ...
  let candidateIds = ((pool ?? []) as any[]).map((p) => p.id)
  const nameById = new Map(((pool ?? []) as any[]).map((p) => [p.id, p.name]))
  if (candidateIds.length === 0) return []

  // Dedupe: drop anyone already notified for this request.
  const { data: already } = await db.from('sub_request_notifications').select('user_id').eq('request_id', requestId).in('user_id', candidateIds)
  ...
  return candidateIds.slice(0, limit).map((id) => ({ userId: id, name: nameById.get(id) ?? 'A player' }))
}
...
// Record that these users were notified for this request (dedupe). Best-effort; ignores conflicts.
export async function markNotified(requestId: string, userIds: string[]): Promise<void> {
  if (userIds.length === 0) return
  const db = admin()
  await db.from('sub_request_notifications').upsert(
    userIds.map((user_id) => ({ request_id: requestId, user_id })),
